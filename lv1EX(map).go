...
func main() {
	...
	for i := 0; i < len(skillNames); i++ {
		fmt.Println("请输入技能", i+1, "您想选择的模板：")
		fmt.Scan(&m)
		set[models[m-1]] = skillNames[i]
	}
	fmt.Println("请输入您想释放的技能名：")
	fmt.Scan(&skillname)
	Re(skillname, func(skillName string) {
		for i := 0; i < len(skillNames); i++ {
			if skillname == skillNames[i] {
				fmt.Println(models[m-1])
			}
		}
	})
}
func Re(skill string, releaseSkillFunc func(string)) {
